...
class ODSReader(BaseTableReader):
    """
    Reads ODS (OpenDocument Spreadsheet) files.
    ...
    def read_bytes(self, raw_bytes: bytes, stem: str) -> list[RawTable]:
        try:
            xl = pd.ExcelFile(io.BytesIO(raw_bytes), engine="odf")
        except Exception as exc:
            raise ValueError(f"Failed to open ODS file: {exc}") from exc
